...
while($train_line = <$train_file>) {	## accept input from file
	...
	for (my $cnt=2; $cnt<=$#temp_input; $cnt+=2)  {
		...
		$class_word{$temp_input[1]}{$temp_input[$cnt]} += 1;
		$word{$temp_input[$cnt]} = 1;

		$word_class_table{$temp_input[1]}{$temp_input[$cnt]} = $temp_input[$cnt];
	}
}
...
sub calculate_prob {
	...
	foreach $key (sort keys %class) {
		...
		print {$model_file} $key, "\t", ($p_delta + $class{$key})/(($p_delta * $class_label) + $total_class), "\t", log(($p_delta + $class{$key})/(($p_delta * $class_label) + $total_class))/log(10), "\n";
	...
	foreach $key (sort keys %class) {
		...
		foreach $key (sort keys %word) {
			...
			$prob_w = ($c_delta + $class_word{$class_str}{$key})/(($c_delta * 2) + $class{$class_str});
			$not_class_prob{$class_str} += log(1-$prob_w)/log(10);

			if ($prob_w ne 0) {

				$l_prob_w = log($prob_w)/log(10);

				$class_word_prob{$class_str}{$key} = $prob_w;
	...
	foreach $key (sort keys %train_instance) {
		...
		foreach $key (keys %class) {
			...
			for (my $cnt=2; $cnt<=$#temp_input; $cnt+=2) {
				...
				$present_word = $class_word_prob{$class_str}{$temp_input[$cnt]};
				$no_word = 1-$present_word;
			
				if (($present_word/$no_word) ne 0) {

					$current_wordprob = log($present_word/$no_word)/log(10);
				}

				$sum_yprob += $current_wordprob;
			}

			$sum_nprob = $not_class_prob{$class_str};

			$instance_prob = $c_prob + ($sum_yprob + $sum_nprob);
