...
## to run: cat input | eng_tokenizer.pl {dict_file} {word_pattern_file} > output

### created by Fei on 9/30/05.
##
...
### You might want to change the following lines w.r.t. utf
if(0){
if($] !~ /5\.008/){
...
#### step 1: read files
my $word_list_file = "";  ## the list of words that we should not split
if(@ARGV > 0){
...
if($word_list_file ne ""){
    my $suc = open(my $dict_fp, "$word_list_file");
    if(!$suc){
	print STDERR "Warning: $word_list_file doesn't exist\n";
    }else{	
	while(<$dict_fp>){
	    chomp;
	    next if /^\s*$/;
	...
	print STDERR "Finish reading $dict_entry entries in $word_list_file\n";
    }
}
...
if($word_patt_file ne ""){
    if(open(my $patt_fp, "$word_patt_file")){
	while(<$patt_fp>){
	    chomp;
	    next if /^\s*$/;
	    s/^\s+//;
	...
	print STDERR "Warning: $word_patt_file does not exist\n";
    }
}
...
while(<STDIN>){
    ...
    ### Fei: IBM style markups and html markups
