	...
	foreach $word_key(@word_key) {
		...
		if ($char_counter eq $#word_key) {
			...
			if ($#word_key > 0) {
				...
				print "\(q$previous_state $rules[1] \"$word_key\" \"$word_table[$counter]\/$lexicon_table[$counter]\"\)\) \n";
				print "$rules[1] \($end_state *e* *e*\)\) \n";		
			}

			else {
				print "$rules[0] $rules[1] \"$word_key\" \"$word_table[$counter]\/$lexicon_table[$counter]\"\)\) \n";
				print "$rules[1] \($end_state *e* *e*\)\) \n";		
			}
		}

		elsif ($char_counter eq 0) {
			print "$rules[0] \(q$state_counter \"$word_key\" *e*\)\) \n";
			$previous_state = $state_counter;
			$state_counter += 1;
		}

		else {
			print "\(q$previous_state \(q$state_counter \"$word_key\" *e*\)\) \n";
			$previous_state = $state_counter;
			$state_counter += 1;
		}
